...
The optional `<count>` argument, which defaults to 10, is the upper limit of the number of entries that the command attempts to claim.
...
Lastly, claiming a message with `XAUTOCLAIM` also increments the attempted deliveries count for that message.
