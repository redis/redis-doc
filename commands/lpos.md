...
The optional arguments and options are able to modify the command behavior.
...
When `COUNT` is used, it is possible to specify 0 as number of matches, as a way to tell the command we want all the matches found returned as an array of indexes. This is better than giving a very large `COUNT` option because it is more general.
